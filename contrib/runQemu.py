...
def run_x86(image: Path):
    ...
    disk_config = {
            ...
            "partitions": [{
                "name": "Testing",
                "final_lba": 411647,
                "test_script": "ci/nci-config/x86/scripts/target/muenblock-ref",
            },
            {
                "name": "ext",
                "final_lba": 729054,
...
lines = dist_dir.strip().splitlines()
if len(lines) != 1:
...
if "x86" in args.query:
    ...
    os.environ["PATH"] = os.environ["PATH"] + os.pathsep + str(qemu_path.resolve())
