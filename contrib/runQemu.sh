...
echo "Staring qemu for $1 (${DIST_DIR})"
...
exec ${QEMU_PATH}/usr/bin/qemu-system-aarch64  \
   ...
   -chardev pty,id=char1,logfile=serial1.log -serial chardev:char1  \
   -chardev pty,id=char2,logfile=serial2.log -serial chardev:char2  \
   -display none \
